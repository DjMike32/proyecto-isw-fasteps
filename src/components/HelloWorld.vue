...
<template>
  ...
  <ol>
    <li v-for="(value, property, index) in objectFruta" :key="index">
      ...
      <span v-if="property === 'name'"> es una pera!
      </span>
    </li>
  </ol>
  ...
  <button @click="reset" class="bg-purple-500">
    Reiniciar</button> <br>
  ...
  <form @submit.prevent="agregarFruta" class="mt-6  grid flex-col justify-items-center gap-4">
    ...
    <button type="submit" class="bg-yellow-300 px-10 shadow-lg opacity-75 hover:opacity-100 ">Agregar a la lista
    </button>
  </form>
